...
```bash
npx devspace-local-artifacts config set publicBaseUrl https://your-tunnel-host.example.com
```
...
```bash
DEVSPACE_PUBLIC_BASE_URL="https://new-tunnel.example.com" npx devspace-local-artifacts serve
```
...
```bash
DEVSPACE_ALLOWED_HOSTS="*" npx devspace-local-artifacts serve
```
...
```bash
DEVSPACE_OAUTH_ALLOWED_REDIRECT_HOSTS="chatgpt.com,example.com" npx devspace-local-artifacts serve
```
...
```bash
npx devspace-local-artifacts init --force
```
...
```bash
DEVSPACE_SKILLS=1 npx devspace-local-artifacts serve
```
