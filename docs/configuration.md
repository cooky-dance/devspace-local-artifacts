...
```bash
DEVSPACE_CONFIG_DIR=/path/to/config npx devspace-local-artifacts serve
```

## Commands

```bash
npx devspace-local-artifacts init
npx devspace-local-artifacts serve
npx devspace-local-artifacts doctor
npx devspace-local-artifacts config get
npx devspace-local-artifacts config set publicBaseUrl https://devspace.example.com
```
...
```bash
DEVSPACE_ARTIFACTS=1 npx devspace-local-artifacts serve
```

Native-file download supports Linux and Windows. macOS and BSD are not
registered because the secure publication path depends on platform-specific
directory safety primitives that are not available in this release.
...
`write_binary` is a fallback for hosts that cannot send native file parameters.
It accepts one strict Base64 string or Base64 data URL and is capped at 32 MiB.
