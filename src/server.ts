...
export const INLINE_PATCH_MAX_BYTES = 256 * 1024;
...
function serverInstructions(config: ServerConfig): string {
  const artifactInstruction = shouldRegisterArtifactTools(config.artifactsEnabled)
    ? " When the user supplies or generates a file that is not present on the DevSpace host, use download_artifact with its native file value, the existing workspace ID, and a suitable relative destination path chosen from the user's request and project structure. If the host does not expose native file parameters but the complete original Base64 payload or data URL is available, use write_binary with contentBase64; never invent, truncate, or substitute binary content. Both tools refuse to overwrite an existing destination and return the normalized workspace-relative path. Use normal workspace tools when explicit inspection, replacement, movement, renaming, or deletion is needed. Do not recreate binary files with write/edit calls or place signed URLs, native file objects, base64 content, or invented host paths in shell commands or logs."
    : "";
...
export function mcpBodyParserErrorStatus(error: unknown): number | undefined {
  ...
  if (type === "entity.parse.failed" || (error instanceof SyntaxError && "body" in error)) {
    return 400;
  }
  return undefined;
}
...
function countDiffStats(diff: string | undefined): DiffStats {
  ...
  for (let index = 0; index <= diff.length; index++) {
    if (index < diff.length && diff.charCodeAt(index) !== 0x0a) continue;

    const first = diff.charCodeAt(lineStart);
    const header = diff.slice(lineStart, lineStart + 3);
    if (first === 0x2b && header !== "+++") additions++;
    if (first === 0x2d && header !== "---") removals++;
    lineStart = index + 1;
  }
...
export function newFilePatch(
  ...
  const estimatedBytes = Buffer.byteLength(content, "utf8")
    + lineCount
    + Buffer.byteLength(path, "utf8") * 2
    + 256;
  if (estimatedBytes > INLINE_PATCH_MAX_BYTES) return undefined;
  ...
  const patch = [
    ...
    .join("\n");
  return Buffer.byteLength(JSON.stringify(patch), "utf8") <= INLINE_PATCH_MAX_BYTES
    ? patch
    : undefined;
}

function boundedPatch(value: string | undefined): string | undefined {
  if (!value || Buffer.byteLength(JSON.stringify(value), "utf8") > INLINE_PATCH_MAX_BYTES) {
    return undefined;
  }
  return value;
}
...
function createMcpServer(
  ...
  registerAppTool(
    ...
    async ({ workspaceId, ...input }) => {
      ...
      const lines = contentLineCount(input.content);
      const patch = newFilePatch(input.path, input.content, lines);
      const previewOmitted = patch === undefined;
      const stats = { additions: lines, removals: 0 };
      const summary = {
        ...stats,
        lines,
      ...
      return {
        ...
        _meta: {
          ...
          card: {
            ...
            summary,
            payload: {
              content: previewOmitted
                ? [textBlock("File written successfully. Diff preview omitted for performance because it exceeded 256 KiB.")]
                : response.content,
              patch,
            },
          },
  ...
  registerAppTool(
    ...
    async ({ workspaceId, ...input }) => {
      ...
      const sourcePatch = response.details?.patch;
      const sourceDiff = response.details?.diff;
      const preferredPreview = sourcePatch ?? sourceDiff;
      const boundedPreview = boundedPatch(preferredPreview);
      const patch = sourcePatch === preferredPreview ? boundedPreview : undefined;
      const diff = sourceDiff === preferredPreview ? boundedPreview : undefined;
      const previewOmitted = Boolean(preferredPreview) && !boundedPreview;
      const stats = previewOmitted
        ? { additions: 0, removals: 0 }
        : countDiffStats(patch ?? diff);
      const summary = {
        ...stats,
        editCount: input.edits.length,
        previewOmitted,
        statsOmitted: previewOmitted,
      };
      const editResultText = previewOmitted
        ? `Edited ${input.path} (diff statistics omitted for performance).`
      ...
      return {
        ...
        _meta: {
          ...
          card: {
            ...
            payload: {
              content: previewOmitted
                ? [textBlock("File edited successfully. Diff preview omitted for performance because it exceeded 256 KiB.")]
                : undefined,
              diff,
              patch,
            },
  ...
  if (shouldRegisterArtifactTools(config.artifactsEnabled)) {
...
export function createServer(
  ...
  const mcpJsonBodyParser = express.json({
    limit: artifactJsonBodyLimit(inlineArtifactMaxFileBytes(config.artifactMaxFileBytes)),
  });
  ...
  app.all("/mcp", bearerAuth, mcpJsonBodyParser, async (req, res) => {
...
if (await isMainModule()) {
  ...
  const httpServer = app.listen(config.port, config.host, () => {
    ...
    const artifactDownloadStatus = !config.artifactsEnabled
      ? "disabled"
      : shouldRegisterArtifactTools(config.artifactsEnabled)
        ? "enabled"
